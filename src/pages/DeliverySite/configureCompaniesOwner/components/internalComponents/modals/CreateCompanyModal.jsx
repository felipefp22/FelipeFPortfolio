...
export default function CreateCompanyModal({ close, }) {
                ...
                <div className="modalInside" style={{ width: 'auto', padding: '20px',  maxWidth: !isDesktopView ? "95%" : "80%", maxHeight: !isDesktopView ? "95%" : "90%", zIndex: 10, fontSize: !isDesktopView ? '20px' : '26px', }}>
                    <div>
                        <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'center', alignItems: 'center', justifyContent: 'center', alignContent: 'center', lineHeight: 1.8, marginBottom: '10px' }}>
                            ...
                            <span style={{ margin: 10, fontSize: '15px'}}> {"Open to - "} <span style={{ color: blueOne(theme) }}>{openShiftModal?.companyName}</span></span>
                            <span style={{ marginBottom: 10, fontSize: '15px'}}> {"Will be - "} <span style={{ color: blueOne(theme) }}>{`Shift ${openShiftModal?.lastOrOpenShift ? Number(openShiftModal?.lastOrOpenShift.shiftNumber) + 1 : 1}`}</span></span>
                        </div>
